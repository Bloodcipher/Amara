  ...
  const loadData = useCallback(async () => {
    try {
      const [jc, qc, prods, usrs] = await Promise.all([
        fetchJobCards(), fetchQCLogs(), fetchProducts(), fetchUsers(),
      ]);
      setJobCards(jc); setQCLogs(qc); setProducts(prods); setUsers(usrs);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input data-testid="jc-number-input" value={jcForm.job_card_number}
                    onChange={(e) => setJcForm({ ...jcForm, job_card_number: e.target.value })}
                    placeholder="0BSFXSS001-P042"
                    className="w-full bg-neutral-900/50 border border-white/10 rounded-md h-10 px-3 text-sm focus:border-gold/50 outline-none" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={jcForm.assigned_artisan_id}
                    onChange={(e) => setJcForm({ ...jcForm, assigned_artisan_id: e.target.value || null })}
                    className="w-full bg-neutral-900/50 border border-white/10 rounded-md h-10 px-3 text-sm focus:border-gold/50 outline-none">
                ...
                <div>
                  ...
                  <select value={jcForm.priority}
                    onChange={(e) => setJcForm({ ...jcForm, priority: e.target.value })}
                    className="w-full bg-neutral-900/50 border border-white/10 rounded-md h-10 px-3 text-sm focus:border-gold/50 outline-none">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <select data-testid="qc-jobcard-select" value={qcForm.job_card_id}
                    ...
                    {jobCards.map(jc => <option key={jc.id} value={jc.id}>{jc.job_card_number}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-neutral-500 uppercase tracking-wider block mb-1">Inspector</label>
                  <select value={qcForm.inspected_by}
                    onChange={(e) => setQcForm({ ...qcForm, inspected_by: e.target.value || null })}
                    className="w-full bg-neutral-900/50 border border-white/10 rounded-md h-10 px-3 text-sm focus:border-gold/50 outline-none">
                ...
                <div className="md:col-span-2">
                  ...
                  <input value={qcForm.defect_reason}
                    onChange={(e) => setQcForm({ ...qcForm, defect_reason: e.target.value })}
      ...
      <div className="glass-card rounded-lg overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-white/5">
          ...
          <span className="text-sm font-medium">
            {activeTab === 'job-cards' ? `${jobCards.length} Job Cards` : `${qcLogs.length} QC Logs`}
          </span>
        </div>
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                  ...
                  <tr key={jc.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <span className={`text-xs px-2 py-0.5 rounded capitalize
                        ${jc.priority === 'urgent' ? 'bg-red-500/20 text-red-400' :
                          jc.priority === 'high' ? 'bg-orange-500/20 text-orange-400' : 'bg-white/5 text-neutral-400'}`}>
                        {jc.priority}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <span className={`text-xs px-2 py-0.5 rounded capitalize ${STATUS_COLORS[jc.status] || ''}`}>
                        {jc.status.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <select
                        data-testid={`status-select-${jc.id}`}
                        value={jc.status}
                ...
                {jobCards.length === 0 && (
                  <tr><td colSpan={8} className="py-8 text-center text-neutral-500 text-sm">No job cards yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {qcLogs.map(q => {
                  ...
                  return (
                    <tr key={q.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <span className={`text-xs font-mono ${parseFloat(rate) >= 90 ? 'text-emerald-400' : 'text-amber-400'}`}>{rate}%</span>
                      </td>
                ...
                {qcLogs.length === 0 && (
                  <tr><td colSpan={7} className="py-8 text-center text-neutral-500 text-sm">No QC logs yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
